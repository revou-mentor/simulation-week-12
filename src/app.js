...
window.addEventListener('DOMContentLoaded', () => {
  ...
  quoteButton.addEventListener('click', () => {
    fetch('https://api.api-ninjas.com/v1/quotes?category=happiness', {
      headers: {'X-Api-Key': 'tOj7dnA5FiVG9cekJkWjJg==xcyDJ9Uqe9HJonxS'},
    })
      .then((response) => response.json())
      .then((data) => {
        quoteContainer.innerText = data[0].quote ?? 'Tidak ada quote hari ini';
      })
      .catch((err) => {
        quoteContainer.innerText = 'Tidak ada quote hari ini';
      });
  });
});
